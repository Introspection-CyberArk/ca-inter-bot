...
class CAContent:
    def __init__(self):
        ...
        self.mcqs = {
            'acc_1': [
                {'id': 'acc_1_1', 'question': 'Which accounting standard deals with PPE?', 
                 'options': ['AS 10', 'AS 16', 'AS 7', 'AS 19'], 'correct_answer': 0, 
                 'explanation': 'AS 10 deals with Property, Plant and Equipment.'}
            ],
            'law_1': [
                {'id': 'law_1_1', 'question': 'Minimum directors in a public company?', 
                 'options': ['2', '3', '5', '7'], 'correct_answer': 1, 
                 'explanation': 'Section 149 requires minimum 3 directors.'}
            ],
            'tax_1': [
                {'id': 'tax_1_1', 'question': 'Residential status is based on:', 
                 'options': ['Citizenship', 'Domicile', 'Physical presence', 'All'], 
                 'correct_answer': 2, 'explanation': 'Based on physical presence in India.'}
            ]
        }
    ...
    def explain_concept(self, query):
        q = query.lower()
        if 'depreciation' in q: return "📘 DEPRECIATION: Systematic allocation of cost over useful life."
        if 'gst' in q: return "📘 GST: Comprehensive indirect tax with CGST, SGST, IGST."
        return "📘 CONCEPT: Important CA Inter topic. Refer ICAI module."
...
class CABot:
    ...
    async def start(self, update, context):
        ...
        self.user_sessions[user.id] = {'current_subject': None, 'mcqs': [], 'current_mcq': 0, 'score': 0, 'total': 0, 'history': []}
        keyboard = [
    ...
    async def display_mcq(self, update, context, user_id):
        ...
        keyboard.append([InlineKeyboardButton("📊 Progress", callback_data="show_progress")])
    ...
    async def handle_test_answer(self, update, context):
        ...
        mcq = session['test_mcqs'][session['test_index']]
        is_correct = selected == mcq['correct_answer']
        if is_correct: session['test_score'] += 1
    ...
    async def show_test_results(self, update, context, user_id):
        ...
        await update.callback_query.edit_message_text(
            f"📊 TEST COMPLETE!\n\n✅ Correct: {score}/{total}\n📈 Score: {(score/total*100):.1f}%\n\n{'🌟 EXCELLENT!' if score/total >= 0.7 else '💪 KEEP PRACTICING!'}",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔄 Retry Test", callback_data="practice_test"), InlineKeyboardButton("📚 Back", callback_data="back_main")]]),
            parse_mode='Markdown')
...
def main():
    ...
    app.add_handler(ConversationHandler(
        entry_points=[CallbackQueryHandler(bot.ask_doubt, pattern="^ask_doubt$")],
